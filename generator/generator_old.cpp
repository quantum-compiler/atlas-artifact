...
void Generator::generate(int num_qubits,
  ...
  dfs(0, max_num_gates, dag, used_parameters, dataset_prev);
...
void Generator::dfs(int gate_idx,
  ...
  if (save_into_dataset) {
    ...
    dataset_prev.insert(context, dag->clone_and_shrink_unused_input_parameters());
  }

  // check that this circuit is different with any other circuits in the dataset_prev
  for (auto &other_dag : dataset_prev[dag->hash(context)]) {
        ...
        for (int p1 = 0; p1 < dag->get_num_total_parameters(); p1++) {
          ...
          for (int p2 = 0; p2 < dag->get_num_total_parameters(); p2++) {
            ...
            bool ret = dag->add_gate(qubit_indices,
                                     ...
                                     &output_param_index);
            assert(ret);
            dfs(gate_idx + 1, max_num_gates, dag, used_parameters, dataset_prev);
