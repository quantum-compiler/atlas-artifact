...
void DistributedSimulator::shuffle_task(
  ...
  GenericTensorAccessorW gpu_state_vector = helperGetGenericTensorAccessorWO(
      handler->vecDataType, regions[0], task->regions[0], FID_DATA, ctx,
      runtime);
  GenericTensorAccessorR cpu_sv[MAX_CPU_SV_INPUT];
  for (int i = 0; i < regions.size() - 1; i++) {
    cpu_sv[i] = helperGetGenericTensorAccessorRO(
        handler->vecDataType, regions[i + 1], task->regions[i + 1], FID_DATA,
        ctx, runtime);
  }
  ...
  for (int i = 0; i < regions.size() - 1; i++) {
    cudaMemcpyAsync(
        (void *)(gpu_state_vector.get_void_ptr() +
                 i * sizeof(handler->vecDataType) * domain.get_volume()),
...
void DistributedSimulator::store_task(
  ...
  GenericTensorAccessorW cpu_sv = helperGetGenericTensorAccessorWO(
      handler->vecDataType, regions[1], task->regions[1], FID_DATA, ctx,
      runtime);
  ...
  cudaMemcpyAsync(cpu_sv.get_void_ptr(), gpu_state_vector.get_void_ptr(),
                  sizeof(handler->vecDataType) * domain.get_volume(),
...
void DistributedSimulator::sv_comp_task(
  ...
  DSHandler const *handler = (DSHandler *)task->local_args;
  assert(handler->vecDataType == DT_FLOAT_COMPLEX ||
         handler->vecDataType == DT_DOUBLE_COMPLEX);
  cudaDataType_t data_type =
      handler->vecDataType == DT_FLOAT_COMPLEX ? CUDA_C_32F : CUDA_C_64F;
  custatevecComputeType_t compute_type =
      handler->vecDataType == DT_FLOAT_COMPLEX ? CUSTATEVEC_COMPUTE_32F
  ...
  for (int task_id = 0; task_id < info->num_tasks; task_id++) {
    if (info->task_map[task_id] == FUSED) {
      ...
      custatevecApplyMatrix(
          ...
          /* const void* */
          (void *)&mat[handler->chunk_id * (1 << MAX_KERNEL_SIZE)],
          /* cudaDataType_t */ data_type,
      ...
      checkCUDA(cudaMemcpyAsync(handler->threadBias, hostThreadBias,
                                sizeof(hostThreadBias), cudaMemcpyHostToDevice,
                                stream));
      ...
      for (size_t i = 0; i < num_kgates; i++) {
        hostGates[i] = getGate(kernelgates[i], handler->chunk_id, logicQubitset,
                               qubit_group_map, info->pos, n_local);
      }

      qindex gridDim = (qindex(1) << n_local) >> SHARED_MEM_SIZE;

      copyGatesToSymbol(hostGates, num_kgates, stream, 0);
      LegionApplyGatesSHM(gridDim, (qComplex *)state_vector.get_void_ptr(),
                          handler->threadBias, n_local, num_kgates, blockHot,
                          enumerate, stream, loIdx_device, shiftAt_device);
  ...
  custatevecSwapIndexBits(handler->statevec, state_vector.get_void_ptr(),
                          data_type, nIndexBits, LocalIndexBitSwaps.data(),
                          info->nLocalSwaps, maskBitString, maskOrdering,
                          maskLen);
...
KernelGate sim::getGate(const KernelGate &gate, int part_id,
                        qindex relatedLogicQb, const std::map<int, int> &toID,
                        const unsigned *pos, unsigned n_local) {
  qComplex mat_[2][2] = {
