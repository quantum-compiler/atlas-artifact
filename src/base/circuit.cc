...
template <typename DT>
qcircuit::Circuit<DT>::Circuit(unsigned nqubits, unsigned nlocal, int ndevices,
...
bool qcircuit::Circuit<DT>::compile(quartz::CircuitSeq *seq,
  ...
  //     /*fusion_kernel_costs=*/{0, 10.4, 10.400001, 10.400002, 11, 40, 46,
  //     66},
  //     /*shared_memory_init_cost=*/10,
  //     /*shared_memory_gate_cost=*/[](quartz::GateType type) { if (type ==
  //     quartz::GateType::swap) return 1000.0; else return 0.8; },
  //     /*shared_memory_total_qubits=*/10,
  //     /*shared_memory_cacheline_qubits=*/3);
  quartz::KernelCost kernel_cost(
  ...
  // /*absorb_single_qubit_gates=*/true, -1,
  //                               cache_file_name_prefix);
  auto schedules = get_schedules_with_ilp(
      *seq, n_local, std::min(2, int(num_qubits - n_local)), kernel_cost, ctx,
      interpreter,
      ...
      if (kernel.type == quartz::KernelType::fusion) {
        ...
        for (auto &gate : kernel.gates->gates) {
          ...
          if (gate->gate->get_num_control_qubits() == 2) {
            ...
            qComplex mat_[2][2] = {
                mat[3 * 8 + 3].real(), mat[3 * 8 + 3].imag(),
                mat[3 * 8 + 7].real(), mat[3 * 8 + 7].imag(),
                mat[7 * 8 + 3].real(), mat[7 * 8 + 3].imag(),
                mat[7 * 8 + 7].real(), mat[7 * 8 + 7].imag()};
            ...
            KernelGate kg(toKernel(gate->gate->tp), qubit_indices[1],
                          isGlobalControl2, qubit_indices[0], isGlobalControl1,
                          qubit_indices[2], isGlobalTarget, mat_);
            kernelgates.push_back(kg);
          } else if (gate->gate->get_num_control_qubits() == 1) {
            // compress mat
            qComplex mat_[2][2] = {
                mat[1 * 4 + 1].real(), mat[1 * 4 + 1].imag(),
                mat[1 * 4 + 3].real(), mat[1 * 4 + 3].imag(),
                mat[3 * 4 + 1].real(), mat[3 * 4 + 1].imag(),
                mat[3 * 4 + 3].real(), mat[3 * 4 + 3].imag()};
            qindex mask = active_qubits_logical;
            char isGlobalControl1 = (mask >> qubit_indices[0]) & 1;
            char isGlobalTarget = (mask >> qubit_indices[1]) & 1;
            KernelGate kg(toKernel(gate->gate->tp), qubit_indices[0],
                          isGlobalControl1, qubit_indices[1], isGlobalTarget,
                          mat_);
            kernelgates.push_back(kg);
            ...
            qComplex mat_[2][2] = {mat[0].real(), mat[0].imag(), mat[1].real(),
                                   mat[1].imag(), mat[2].real(), mat[2].imag(),
                                   mat[3].real(), mat[3].imag()};
            qindex mask = active_qubits_logical;
...
template <typename DT> void qcircuit::Circuit<DT>::simulate(bool use_mpi) {
  ...
  for (auto &task : task_map) {
    if (task == SHUFFLE) {
      ...
      //   }
      // }
      simulator.ApplyKernelGates(shm_gates[shm_idx],
                                 active_logical_qs[shm_idx]);
      shm_idx++;
    }
  }

  printf(
      "Finish Simulating! Total: %d FUSE Kernel, %d SHM Kernel, %d Shuffles.\n",
...
bool qcircuit::Circuit<DT>::FuseGates(const quartz::Kernel &kernel,
  ...
  for (int i = 0; i < n_local; i++) {
    if (std::find(kernel.qubits.begin(), kernel.qubits.end(), permutation[i]) !=
        kernel.qubits.end()) {
  ...
  for (int i = 0; i < kernel.gates->gates.size(); i++) {
    ...
    for (int d = 0; d < n_devices; d++) {
      ...
      if (getMat_per_device(ctx, myRank * n_devices + d,
                            kernel.gates->gates[i]->gate, qubit_indices, params,
                            temp_mat_, mask, qperm)) {
        // do MatMul
...
template <typename DT>
bool qcircuit::Circuit<DT>::getMat_per_device(
    quartz::Context *ctx, int part_id, quartz::Gate *gate,
    std::vector<int> qubit_indices, std::vector<ParamType> &params,
    std::vector<ComplexType> &res, unsigned &mask, std::vector<int> &perm) {
  if (kDebug) {
    printf("Fusing gate %d(%d) for rank %d\n", gate->tp,
           gate->get_num_control_qubits(), part_id);
  }
  if (gate->get_num_control_qubits() == 2) { // ccx
    ...
    if (local_mask[c1] && local_mask[c2]) { // CCU(c1, c2, t)
      ...
      for (int t = 0; t < qubit_indices.size(); t++) {
        int it2 =
            std::find(position.begin(), position.end(), sorted_position_[t]) -
            position.begin();
        perm[it2] = t;
      }
      return true;
      ...
      if (IS_HIGH_PART(part_id, c2)) { // CU(c1, t)
        ...
        perm.resize(2);
        perm[0] = qubit_group_map_fusion.at(c1) > qubit_group_map_fusion.at(t)
                      ? 1
                      : 0;
        perm[1] = 1 - perm[0];
        return true;
    ...
    if (local_mask[c] && local_mask[t]) { // CU(c, t)
      ...
      perm.resize(2);
      perm[0] =
          qubit_group_map_fusion.at(c) > qubit_group_map_fusion.at(t) ? 1 : 0;
      perm[1] = 1 - perm[0];
      return true;
      ...
      // printf("here4\n");
      assert(gate->tp == quartz::GateType::cz ||
             gate->tp == quartz::GateType::cp);
...
void qcircuit::Circuit<DT>::update_layout(std::vector<int> targets) {
  ...
  for (int i = 0; i < n_global; i++) {
    ...
    if (pos[targets[i + n_local]] < n_local)
      num_swaps++;
  }
      ...
      if (new_global_pos[i] >= (n_local - num_swaps)) {
        ...
        for (int j = num_swaps - 1; j >= 0; j--) {
          if (!(local_mask >> j & 1)) {
            ...
            std::swap(pos[permutation[new_global_pos[i]]],
                      pos[permutation[n_local - num_swaps + j]]);
            std::swap(permutation[new_global_pos[i]],
                      permutation[n_local - num_swaps + j]);
            local_mask |= 1 << j;
            break;
          }
        }
  ...
  for (int i = 0; i < n_global; i++) {
    if ((~global_mask) >> i & 1) {
      std::swap(pos[permutation[n_local - nGlobalSwaps]],
...
template <typename DT>
void qcircuit::Circuit<DT>::update_layout_from_schedule(
    const quartz::Schedule &cur_schedule,
    const quartz::Schedule &prev_schedule) {

  std::vector<int2> local_swap;
  unsigned global = 0;
  auto local_swap_from_schedule =
      cur_schedule.get_local_swaps_from_previous_stage(prev_schedule);
