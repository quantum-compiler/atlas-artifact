...
void sim::top_level_task(Task const *task,
  ...
  auto seq = quartz::CircuitSeq::from_qasm_file(
      &qtz, atlas_home + std::string("/circuit/MQTBench_") +
                ...
                "_indep_qiskit_" + std::to_string(config.num_all_qubits) +
                "_no_swap.qasm");
  sim::qcircuit::Circuit<double> circuit(
      config.num_all_qubits, config.num_local_qubits,
      (unsigned(1) << (config.num_all_qubits - config.num_local_qubits)), 0, 1);
...
int main(int argc, char **argv) {
  ...
  setenv("PYTHONPATH", "~/.conda/envs/pulp/lib/python3.8/site-packages/",
         true /*overwrite*/);
  quartz::init_python_interpreter();
  ...
  Runtime::set_top_level_task_id(TOP_LEVEL_TASK_ID);
  {
    ...
    Runtime::preregister_task_variant<sim::top_level_task>(registrar,
                                                           "top_level");
  }
  {
    ...
    registrar.set_leaf();
    Runtime::preregister_task_variant<DistributedSimulator::DSHandler,
                                      DistributedSimulator::cuda_init_task>(
