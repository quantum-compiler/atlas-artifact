...
def simulate(circuit_name, nqubits, cusvaer_enable=True):
    ...
    if cusvaer_enable:
        ...
        with open(f"logs/cusvaer/{circuit_name}_{nqubits}_{result.mpi_rank}.log",
                  "w") as f:
        ...
        with open(
                f"logs/qiskit/{circuit_name}_{nqubits}_{result.metadata['mpi_rank']}.log",
                "w") as f:
            f.write(f"{1000 * (end_time - start_time)}ms\n")
            # f.write(f"{1000 * result.time_taken}ms\n")
